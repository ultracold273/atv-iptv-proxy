...
- **FR-013**: The admin page MUST allow editing provider settings: User ID, Password, STB ID, Local IP, Local MAC, Auth server URL.
...
- **FR-033**: The proxy MUST cache the channel list with a configurable TTL.
- **FR-034**: The proxy SHOULD cache EPG responses with a shorter configurable TTL.
- **FR-035**: The proxy MUST serve fresh cache without calling the backend.
- **FR-036**: The proxy MAY serve stale cache when backend refresh fails, but MUST mark the response as stale.
- **FR-037**: The proxy MUST write cache updates atomically so interrupted refreshes do not corrupt existing cache.
- **FR-038**: Persistent cache files MUST NOT contain provider password, raw client tokens, authenticator hex, or session cookies unless a later design explicitly encrypts them.

### Testing & Observability

- **FR-039**: The proxy MUST include unit tests for token generation/validation, config redaction, cache TTL decisions, and CTC response parsing.
- **FR-040**: The proxy MUST include integration tests with a mock IPTV backend covering login, channel fetch, cache hit, cache miss, stale fallback, and token rejection.
- **FR-041**: Logs MUST include request outcome, cache hit/miss, refresh outcome, and token client name when known.
- **FR-042**: Logs MUST NOT include provider password, raw local tokens, backend session IDs, or authenticator values.
...
- **SC-005**: Test coverage includes backend mock flows and cache behavior sufficient to prevent regressions in auth/session/channel import.
- **SC-006**: A recursive search of logs and persisted config/cache shows no plaintext provider password, raw local token, backend session ID, or authenticator hex.
...
- Streaming/transcoding media data through the proxy unless later analysis proves stream URLs must be rewritten.
